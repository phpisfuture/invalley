...
class Image
{
    ...
    protected function __construct(\SplFileInfo $file)
    {
        ...
        if (false === $info || (IMAGETYPE_GIF === $info[2] && empty($info['bits']))) {
            throw new ErrorException('Illegal image file');
        }
        ...
        if (empty($this->im)) {
            throw new ErrorException('Failed to create image resources!');
        }
    ...
    public static function open($file)
    {
        ...
        if (!$file->isFile()) {
            throw new ErrorException('image file not exist');
        }
        ...
        do {
            ...
            switch ($direction) {
                ...
                case self::FLIP_Y:
                    ...
                    break;
                default:
                    throw new ErrorException('不支持的翻转类型');
            }
    ...
    public function thumb($width, $height, $type = self::THUMB_SCALING)
    {
        ...
        switch ($type) {
            ...
            case self::THUMB_FIXED:
                ...
                break;
            default:
                throw new ErrorException('不支持的缩略图裁剪类型');
        }
        /* 裁剪图像 */
    ...
    {
        if (!is_file($source)) {
            throw new ErrorException('水印图像不存在');
        }
        //获取水印图像信息
        $info = getimagesize($source);
        if (false === $info || (IMAGETYPE_GIF === $info[2] && empty($info['bits']))) {
            throw new ErrorException('非法水印文件');
        }
        //创建水印图像资源
        ...
        switch ($locate) {
            ...
            default:
                ...
                if (is_array($locate)) {
                    ...
                    throw new ErrorException('不支持的水印位置类型');
                }
        }
        do {
            //添加水印
    ...
    public function text($text, $font, $size, $color = '#00000000',
        ...
        if (!is_file($font)) {
            throw new ErrorException("不存在的字体文件：{$font}");
        }
        //获取文字信息
        ...
        switch ($locate) {
            ...
            default:
                ...
                if (is_array($locate)) {
                    ...
                    throw new ErrorException('不支持的文字位置类型');
                }
        }
        /* 设置偏移量 */
        if (is_array($offset)) {
        ...
        if (is_string($color) && 0 === strpos($color, '#')) {
            ...
            throw new ErrorException('错误的颜色值');
        }
        do {
            /* 写入文字 */
